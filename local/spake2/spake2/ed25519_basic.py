...
def scalarmult_element(pt, n):  # extended->extended
    ...
    el = _add_elements_nonunfied(_, pt) if n & 1 else _
    return el
...
def decodepoint(s):
    # print('Encoded point is ' + bytes_to_str(s))
    unclamped = int(binascii.hexlify(s[:32][::-1]), 16)
    # print('Unclamped is ' + str(unclamped))
    clamp = (1 << 255) - 1
...
def arbitrary_element(seed):  # unknown DL
    ...
    # print('seed ' + str(seed) + ' expands to y ' + str(y))
...
def bytes_to_element(bytes):
    # print('Decoding element from bytes ' + bytes_to_str(bytes))
