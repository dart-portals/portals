...
execfile('spake2/params.py')
execfile('spake2/ed25519_group.py')
...
def finalize_SPAKE2_symmetric(idSymmetric, msg1, msg2, K_bytes, pw):
    ...
    # print('first_msg = ' + bytes_to_str(first_msg))
    # print('second_msg = ' + bytes_to_str(second_msg))
    transcript = b"".join([sha256(pw).digest(),
                           sha256(idSymmetric).digest(),
                           first_msg, second_msg, K_bytes])
    # print('transcript:')
    # print('  pw = ' + bytes_to_str(sha256(pw).digest()))
    # print('  idSymmetric = ' + bytes_to_str(sha256(idSymmetric).digest()))
    # print('  firstMsgBytes = ' + bytes_to_str(first_msg));
    # print('  secondMsgBytes = ' + bytes_to_str(second_msg));
    # print('  kBytes = ' + bytes_to_str(K_bytes));
    key = sha256(transcript).digest()
    return key
...
class _SPAKE2_Base:
    ...
    def start(self):
        ...
        self.xy_scalar = 6019267352642718384477389175057995968320597738885492113584218693522637723914
        print('xy_scalar = ' + str(self.xy_scalar))
        self.xy_elem = g.Base.scalarmult(self.xy_scalar)
        print('xyElem = ' + str(self.xy_elem))
    ...
    def compute_outbound_message(self):
        print('\nmy_blinding = ' + str(self.my_blinding()))
        print('pw_scalar = ' + str(self.pw_scalar))
        pw_blinding = self.my_blinding().scalarmult(self.pw_scalar)
        print('\npw_blinding = ' + str(pw_blinding))
        message_elem = self.xy_elem.add(pw_blinding)
        self.outbound_message = message_elem.to_bytes()
        print('\nmessageElem = ' + str(message_elem))
        print('messageElem affine: ' + str(xform_extended_to_affine(message_elem.XYTZ)))
        print('messageElem bytes = ' + bytes_to_str(self.outbound_message))
    ...
    def finish(self, inbound_side_and_message):
        ...
        # print('my_unblinding = ' + str(self.my_unblinding()))
        # print('pw_unblinding = ' + str(pw_unblinding))
        # print('xy_scalar = ' + str(self.xy_scalar))
        # print('pw_scalar = ' + str(self.pw_scalar))
        # print('inbound_message = ' + bytes_to_str(self.inbound_message))
        # print('inbound_elem = ' + str(inbound_elem))
        K_elem = inbound_elem.add(pw_unblinding).scalarmult(self.xy_scalar)
        # print('\nkElem = ' + str(K_elem))
        K_bytes = K_elem.to_bytes()
        print('K_bytes = ' + bytes_to_str(K_bytes))
...
class SPAKE2_Symmetric(_SPAKE2_Base):
    ...
    def _extract_message(self, inbound_side_and_message):
        ...
        print('other_side = "' + str(other_side) + '"')
        print('SideSymmetric = ' + str(SideSymmetric))
